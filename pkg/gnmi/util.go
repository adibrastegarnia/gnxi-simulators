...
// Flatten make a nested map to a flat map.
func (s *Server) Flatten(aMap map[string]interface{}) map[string]interface{} {
	...
	for key, val := range aMap {
		...
		case map[string]interface{}:
			...
			for nk, nv := range nm {
				flattenMapKey := key + "/" + nk

				log.Info(flattenMapKey)
				flattenMap[flattenMapKey] = nv
			}
		case []interface{}:

			// TODO
		default:
			//log.Info(flattenMap[key])
			flattenMap[key] = val
			//log.Info(val)
		}
	}
...
func (s *Server) ParseJSONTree(aMap map[string]interface{}, path []string, pathFlag bool, dataType string) error {
	...
	for key, val := range aMap {
		...
		case map[string]interface{}:
			...
			s.ParseJSONTree(val.(map[string]interface{}), path, pathFlag, dataType)
			if strings.Compare(key, strings.ToLower(dataType)) == 0 {
				log.Info("key1:", key)
				//delete(val.(map[string]interface{}), key)
		...
		for _, k := range d.MapKeys() {
			...
			if match || matchAll == 0 {
				...
				if typeOfValue == reflect.Map {
					...
					//log.Info(len(tempMap))
					if len(tmpMap) != 0 {
						//tmpData[k.String()] = iterate(d.MapIndex(k).Interface(), dataType)
						tmpData[k.String()] = tmpIteration
						if Contains(dataTypes, k.String()) {
							//log.Info("before:", tmpData)
							delete(tmpData, k.String())
							//log.Info("after:", tmpData)
						}
					}
					...
					//log.Info(k)
					log.Info(checkPathContainType(fullPath, dataType))
					if checkPathContainType(fullPath, dataType) {
