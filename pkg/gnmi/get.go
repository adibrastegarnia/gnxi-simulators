...
func (s *Server) Get(ctx context.Context, req *pb.GetRequest) (*pb.GetResponse, error) {
	...
	for i, path := range paths {
		...
		//s.ParseJSONTree(jsonTree, pathString, false, dataTypeString)
		result := s.Flatten(jsonTree)

		log.Info(result)
		//log.Info(result, fullPathElements, fullPath)
		rootPath := "/"
		for _, elem := range fullPathElements {
			rootPath = rootPath + elem.GetName() + "/"
			if elem.GetKey() != nil {
				log.Info(elem)
			}
		...
		/*for key, _ := range result {
			arrayStrings := strings.Split(key, "/")
			if arrayStrings[len(arrayStrings)-2] != strings.ToLower(dataType.String()) {
				//log.Info(arrayStrings[len(arrayStrings)-2], ":", key, ":", val)
				//rootPath := rootPath + key
				//log.Info(rootPath)
				//removedPath1, _ := ToGNMIPath(rootPath)
		...
		//newPath1 := "/system/openflow/controllers/controller/main/connections/connection/0/state/aux-id"
		newPath1 := "/system/openflow/controllers/controller/[name=main]/connections/connection/[aux-id=0]/state/aux-id"
		removedPath1, err := ToGNMIPath(newPath1)
		if err != nil {
			log.Info(err)
		} else {
			log.Info("The path that should be removed ", removedPath1)
			...
			log.Info("after first delete:", updatedMap)
		}*/
		...
		log.Info("after the second delete", updatedMap)

		newPath3 := "/system/config/"
		removedPath3, _ := ToGNMIPath(newPath3)
		log.Info("The path that should be removed ", removedPath3)

		s.doDeletePath(updatedMap, prefix, removedPath3)

		log.Info("after the third delete", updatedMap)*/
		//}
